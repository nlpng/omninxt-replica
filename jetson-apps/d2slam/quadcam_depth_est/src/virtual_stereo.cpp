...
VirtualStereo::VirtualStereo(int _cam_idx_a, int _cam_idx_b,
  ...
  auto cam_param_left =
      static_cast<const camodocal::PinholeCamera*>(undist_left->cam_side.get())
          ->getParameters();
  printf("[Debug]camera_left size:%d %d\n", cam_param_left.imageWidth(),
         cam_param_left.imageHeight());
  this->img_size =
      cv::Size(cam_param_left.imageWidth(), cam_param_left.imageHeight());
  cv::Mat K_left =
      (cv::Mat_<double>(3, 3) << cam_param_left.fx(), 0, cam_param_left.cx(), 0,
       cam_param_left.fy(), cam_param_left.cy(), 0, 0, 1);
  auto cam_param_right = static_cast<const camodocal::PinholeCamera*>(
                             _undist_right->cam_side.get())
                             ->getParameters();
  // img_size = cv::Size(cam_param.imageWidth(), cam_param.imageHeight());
  cv::Mat K_right =
      (cv::Mat_<double>(3, 3) << cam_param_right.fx(), 0, cam_param_right.cx(),
       0, cam_param_right.fy(), cam_param_right.cy(), 0, 0, 1);
...
int32_t VirtualStereo::rectifyImage(const cv::Mat& left, const cv::Mat& right,
  ...
  cv::cuda::remap(img_cuda_l, rect_left, cuda_lmap_1, cuda_lmap_2,
                  cv::INTER_LINEAR);
  cv::cuda::remap(img_cuda_r, rect_right, cuda_rmap_1, cuda_rmap_2,
                  cv::INTER_LINEAR);
  return 0;
}
