...
int32_t HitnetTrt::init(const std::string& onnx_model_path,
                        const std::string& trt_engine_path,
                        int32_t stream_number) {
  if (stream_number <= 0) {
    stream_number = 1;
  }
  stream_number_ = stream_number;

  // Check TRT engine file can be loaded, if not create engine from onnx model
  // and save to trt_engine_path
  ...
  for (int32_t i = 0; i < stream_number_; i++) {
    ...
    int32_t ret =
        excutor.init(this->nv_engine_ptr_, std::string(kInputTensorName),
                     std::string(kOutputTensorName));
    if (ret != 0) {
...
int32_t HitnetTrt::deserializeEngine(const std::string& trt_engine_path) {
  ...
  if (engine_file.is_open()) {
    ...
    this->nv_engine_ptr_ = std::shared_ptr<nvinfer1::ICudaEngine>(
        nv_runtime->deserializeCudaEngine(engine_data, size, nullptr));
    delete[] engine_data;
...
int32_t HitnetTrt::buildEngine(const std::string& onnx_model_path,
                               const std::string& trt_engine_path) {
  ...
  profile->setDimensions(kInputTensorName, nvinfer1::OptProfileSelector::kOPT,
                         nvinfer1::Dims4{1, 1, 240, 320});
  ...
  if (engine_file.is_open()) {
    engine_file.write(static_cast<const char*>(engine_data->data()),
                      engine_data->size());
    engine_file.close();
...
int32_t HitnetExcutor::init(std::shared_ptr<nvinfer1::ICudaEngine> engine_ptr,
  ...
  this->buffer_manager_ptr_ = std::make_unique<tensorrt_buffer::BufferManager>(
      this->engine_ptr_, 0, this->nv_context_ptr_);
  ...
  this->input_index_ =
      this->engine_ptr_->getBindingIndex(input_tensor_name.c_str());
  ...
  this->input_size_ = input_dim.d[0] * input_dim.d[1] * input_dim.d[2] *
                      input_dim.d[3] * sizeof(float);
  this->output_index_ =
      this->engine_ptr_->getBindingIndex(output_tensor_name.c_str());
  if (this->output_index_ < 0) {
    std::cout << "getBindingIndex failed" << std::endl;
    return -5;
  }
  auto output_dim =
      this->engine_ptr_->getBindingDimensions(this->output_index_);
  this->output_size_ = output_dim.d[0] * output_dim.d[1] * output_dim.d[2] *
                       output_dim.d[3] * sizeof(float);
  return 0;
}

int32_t HitnetExcutor::setInputImages(const cv::Mat& input) {
  memcpy(this->buffer_manager_ptr_->getHostBuffer(this->input_tensor_name_),
         input.data, this->input_size_);
  buffer_manager_ptr_->copyInputToDeviceAsync(this->stream_);
  return 0;
}

int32_t HitnetExcutor::doInference() {
  bool status = this->nv_context_ptr_->enqueueV2(
      this->buffer_manager_ptr_->getDeviceBindings().data(), this->stream_,
      nullptr);
  if (!status) {
...
int32_t HitnetExcutor::getOutput(cv::Mat& output) {
  memcpy(output.data,
         this->buffer_manager_ptr_->getHostBuffer(this->output_tensor_name_),
         this->output_size_);
  return 0;
}
}  // namespace TensorRTHitnet
