...
//     std::pair<camodocal::CameraPtr, Swarm::Pose> readCameraConfig(const
//     std::string & camera_name, const YAML::Node & config, int32_t
//     extrinsic_parameter_type = 1);
// };
...
QuadcamDepthEstTrt::QuadcamDepthEstTrt(ros::NodeHandle& nh) : nh_(nh) {
  ...
  std::string config_dir =
      config_file_path.substr(0, config_file_path.find_last_of("/"));
  ...
  this->raw_image_process_rate_ =
      std::make_unique<ros::Rate>(ros::Rate(this->fps_));
  ...
  // subscribe
  image_transport::TransportHints hints(this->image_format_,
                                        ros::TransportHints().tcpNoDelay(true));
  image_transport_ = new image_transport::ImageTransport(nh_);
  image_sub_ = image_transport_->subscribe(
      this->image_topic_, 1, &QuadcamDepthEstTrt::quadcamImageCb, this, hints);
  if (enable_texture_) {
...
void QuadcamDepthEstTrt::loadVirtualCameras(YAML::Node& config,
                                            std::string configPath) {
  float avg_brightness = config["avg_brightness"].as<float>();
  std::string photometric_calib_path =
      config["photometric_calib_path"].as<std::string>();
  ...
  if (access(photometric_calib_path.c_str(), F_OK) == 0) {
    ...
    for (int i = 0; i < kCamerasNum; i++) {
      std::string mask_file =
          photometric_calib_path + "/" + std::string("cam_") +
          std::to_string(i) +
          std::string("_vig_mask.png");  // search image "cam_i_vig_mask.png"
      if (access(mask_file.c_str(), F_OK) == 0) {
        photometric_inv_vingette_[i] =
            quadReadVingette(mask_file, avg_brightness);
        printf("[QuadcamDepthEstTrt]: read vignette mask from %s\n",
               mask_file.c_str());
      } else {
        photometric_inv_vingette_[i] =
            cv::Mat(1280, 720, CV_8UC3, cv::Scalar(255, 255, 255));
      }
    }
  } else {
    for (int i = 0; i < kCamerasNum; i++) {
  ...
  std::string cam_calib_file_path =
      config["cam_calib_file_path"].as<std::string>();
  ...
  if (access(cam_calib_file_path.c_str(), R_OK) == 0) {
    ...
    for (const auto& cam_para : fisheye_configs) {
      ...
      auto cam_model = D2FrontEnd::D2FrontendParams::readCameraConfig(
          camera_name, camera_parameters);
      ...
      this->undistortors_.push_back(new D2Common::FisheyeUndist(
          cam_model.first, 0, fov, true,
          D2Common::FisheyeUndist::UndistortPinhole2, this->width_,
          this->height_, photometric_inv_vingette_[photometric_inv_idx]));
    ...
    for (const auto& vstereos : config["stereos"]) {
      ...
      std::string stereo_calib_file =
          stereo_node["stereo_config"].as<std::string>();
      Swarm::Pose baseline;
      ...
      auto stereo = new VirtualStereo(cam_idx_l, cam_idx_r, baseline,
                                      ...
                                      undistortors_[cam_idx_r], idx_l, idx_r);
      auto att = undistortors_[cam_idx_l]->t[idx_l];
      stereo->extrinsic =
          raw_cam_extrinsics_[cam_idx_l] * Swarm::Pose(att, Vector3d(0, 0, 0));
...
std::pair<cv::Mat, cv::Mat> QuadcamDepthEstTrt::intrinsicsFromNode(
...
void QuadcamDepthEstTrt::startAllService() {
  this->raw_image_process_thread_ =
      std::thread(&QuadcamDepthEstTrt::rawImageProcessThread, this);
  this->inference_thread_ =
      std::thread(&QuadcamDepthEstTrt::inferrenceThread, this);
...
void QuadcamDepthEstTrt::quadcamImageCb(
    const sensor_msgs::ImageConstPtr& images) {
  if (!raw_image_mutex_.try_lock()) {
    return;
  } else {
    raw_image_ =
        cv_bridge::toCvCopy(images, sensor_msgs::image_encodings::BGR8)->image;
...
void QuadcamDepthEstTrt::rawImageProcessThread() {
  while (raw_image_process_thread_running_) {
    ...
    for (int32_t i = 0; i < kCamerasNum; i++) {
      cv::Mat splited_image =
          raw_image(cv::Rect(i * raw_image_.cols / kCamerasNum, 0,
      ...
      if (!this->cnn_input_rgb_) {
        ...
        cv::cvtColor(splited_image, split_raw_images_[i],
                     cv::COLOR_BGR2GRAY);  // TODO: Bug openCV segement fault
#ifdef DEBUG
    ...
    for (auto&& stereo : this->virtual_stereos_) {
      char window_name[20];
      cv::Mat show_image;
      cv::Mat left(rectified_images_[stereo->cam_idx_a]
                                    [stereo->cam_idx_a_right_half_id]);
      cv::Mat right(
          rectified_images_[stereo->cam_idx_b][stereo->cam_idx_b_left_half_id]);
      cv::hconcat(left, right, show_image);
      sprintf(window_name, "rectified_image_%d_%d", stereo->cam_idx_a,
              stereo->cam_idx_b);
    ...
    // construct input images for hitnet inferrence and  TODO: can gpu mat be
    // used directly?
    cv::Mat temp_left, temp_right, input_image[4];

    for (auto&& stereo : this->virtual_stereos_) {
      temp_left = cv::Mat(rectified_images_[stereo->cam_idx_a]
                                           [stereo->cam_idx_a_right_half_id]);
      temp_right = cv::Mat(
          rectified_images_[stereo->cam_idx_b][stereo->cam_idx_b_left_half_id]);
    ...
    if (!input_tensors_mutex_.try_lock()) {
      ...
      for (auto&& stereo : this->virtual_stereos_) {
        input_image[stereo->stereo_id].convertTo(
            input_tensors_[stereo->stereo_id], CV_32FC1, 1.0 / 255.0);
      }
...
void QuadcamDepthEstTrt::publishThread() {
  ...
  while (publish_thread_running_) {
    ...
    if (output_tensors_mutex_.try_lock()) {
      for (auto stereo : this->virtual_stereos_) {
        publish_disparity_[stereo->stereo_id] =
            output_tensors_[stereo->stereo_id];
      }
      output_tensors_mutex_.unlock();
    } else {
      this->publish_rate_->sleep();
    ...
    if (show_) {
      ...
      for (auto stereo : this->virtual_stereos_) {
        stereo->showDispartiy(
            publish_disparity_[stereo->stereo_id],
            recity_images_for_show_and_texture_
                [stereo->cam_idx_a][stereo->cam_idx_a_right_half_id],
            recity_images_for_show_and_texture_
                [stereo->cam_idx_b][stereo->cam_idx_b_left_half_id]);
      }
    }
    // prepeare pcl
    if (pcl_ == nullptr) {
    ...
    for (auto stereo : this->virtual_stereos_) {
      cv::Mat points;
      cv::reprojectImageTo3D(publish_disparity_[stereo->stereo_id], points,
                             stereo->getStereoPose(), 3);
      addPointsToPCL(
          points,
          recity_images_for_show_and_texture_[stereo->cam_idx_a]
                                             [stereo->cam_idx_a_right_half_id],
          stereo->extrinsic, *this->pcl_, this->pixel_step_, this->min_z_,
          this->max_z_);
    }
