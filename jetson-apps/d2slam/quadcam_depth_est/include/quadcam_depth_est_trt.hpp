...
class QuadcamDepthEstTrt {
 ...
 private:
  ...
  void publishThread();
  void stoprawImageProcessThread() { raw_image_process_thread_running_ = 0; };
  void stopinfrenceThread() { inference_thread_running_ = 0; };
  void stoppublishThread() { publish_thread_running_ = 0; };
  ...
  std::vector<Swarm::Pose> raw_cam_extrinsics_;  // extrinsics of fisheye
                                                 // cameras
  std::vector<Swarm::Pose>
      virtual_left_extrinsics_;  // extrinsics of virtual left cameras
  ...
  cv::cuda::GpuMat rectified_images_
      [kCamerasNum][2];  //{left,right},{left,right},{left,right},{left,right}
