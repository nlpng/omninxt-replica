...
class VirtualStereo {
 ...
 public:
  ...
  std::vector<cv::cuda::GpuMat> rectifyImage(const cv::Mat& left,
                                             const cv::Mat& right);
  int32_t rectifyImage(const cv::Mat& left, const cv::Mat& right,
                       cv::cuda::GpuMat& rect_left,
                       cv::cuda::GpuMat& rect_right);
  ...
  std::pair<cv::Mat, cv::Mat> estimateDisparityViaRaw(const cv::Mat& left,
                                                      const cv::Mat& right,
                                                      const cv::Mat& left_color,
                                                      bool show = false);
  std::pair<cv::Mat, cv::Mat> estimatePointsViaRaw(const cv::Mat& left,
                                                   const cv::Mat& right,
                                                   const cv::Mat& left_color,
                                                   bool show = false);

  cv::Mat getStereoPose() { return this->Q; }

  int32_t showDispartiy(const cv::Mat& disparity, cv::Mat& left_rect_mat,
                        cv::Mat& right_rect_mat);

  VirtualStereo(int _idx_a, int _idx_b, const Swarm::Pose& baseline,
                D2Common::FisheyeUndist* _undist_left,
                D2Common::FisheyeUndist* _undist_right, int _undist_id_l,
                int _undist_id_r, HitnetONNX* _hitnet,
                CREStereoONNX* _crestereo);
  ...
  VirtualStereo(const Swarm::Pose& baseline, camodocal::CameraPtr cam_left,
                camodocal::CameraPtr cam_right, HitnetONNX* _hitnet,
                CREStereoONNX* _crestereo);
  void initVingette(const cv::Mat& _inv_vingette_l,
                    const cv::Mat& _inv_vingette_r);
  void initRecitfy(const Swarm::Pose& baseline, cv::Mat K0, cv::Mat D0,
                   cv::Mat K1, cv::Mat D1);
};
}  // namespace D2QuadCamDepthEst
