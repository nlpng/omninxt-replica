...
class HitnetExcutor {
 ...
 private:
  ...
  std::shared_ptr<tensorrt_buffer::BufferManager>
      buffer_manager_ptr_;  // TODO: risky
...
class HitnetTrt {
 public:
  ...
  ~HitnetTrt();
  int32_t init(const std::string& onnx_model_path,
               const std::string& trt_engine_path, int32_t stream_number);
  int32_t doInference(
      const cv::Mat input[4]);  // input  4 1x2x240x320 bcwh output 4
                                // 1x1x240x320 disparity
  int32_t getOutput(cv::Mat output[4]);

 private:
  int32_t deserializeEngine(const std::string& trt_engine_path);
  int32_t buildEngine(const std::string& onnx_model_path,
