...
void D2Frontend::stereoImagesCallback(const sensor_msgs::ImageConstPtr left,
  ...
  if (stereo_frame_buffer_lock_.try_lock()) {
    stereo_frame_q_.push(std::make_shared<StereoFrame>(
        _l->header.stamp, _l->image, _r->image, params->extrinsics[0],
        params->extrinsics[1], params->self_id));
...
void D2Frontend::depthImagesCallback(const sensor_msgs::ImageConstPtr left,
  ...
  if (stereo_frame_buffer_lock_.try_lock()) {
    stereo_frame_q_.push(
        std::make_shared<StereoFrame>(left->header.stamp, _l->image, _d->image,
                                      params->extrinsics[0], params->self_id));
    while (stereo_frame_q_.size() > visual_frame_size_) {
      spdlog::warn("D2VINS frontend process is slow; dropping frames\n");
      stereo_frame_q_.pop();
...
void D2Frontend::monoImageCallback(const sensor_msgs::ImageConstPtr &image) {
  ...
  if (stereo_frame_buffer_lock_.try_lock()) {
    stereo_frame_q_.push(std::make_shared<StereoFrame>(
        image->header.stamp, imgs, params->extrinsics, params->self_id));
    while (stereo_frame_q_.size() > visual_frame_size_) {
      spdlog::warn("D2VINS frontend process is slow; dropping frames\n");
      stereo_frame_q_.pop();
    }
...
void D2Frontend::Init(ros::NodeHandle &nh) {
  ...
  if (params->camera_configuration == CameraConfig::STEREO_PINHOLE ||
    ...
    image_sub_l = new ImageSubscriber(*it_, params->image_topics[0], 10, hints);
    image_sub_r = new ImageSubscriber(*it_, params->image_topics[1], 10, hints);
    ...
    image_sub_l = new ImageSubscriber(*it_, params->image_topics[0], 10, hints);
    image_sub_r = new ImageSubscriber(*it_, params->depth_topics[0], 10, hints);
    sync = new message_filters::Synchronizer<ApproSync>(
        ApproSync(10), *image_sub_l, *image_sub_r);
    sync->registerCallback(
  ...
  stereo_frame_thread_rate_ptr_ =
      std::make_unique<ros::Rate>(params->ftconfig->stereo_frame_thread_rate);
  loop_detection_thread_rate_ptr_ =
      std::make_unique<ros::Rate>(params->ftconfig->loop_detection_thread_rate);
  lcm_thread_rate_ptr_ =
      std::make_unique<ros::Rate>(params->ftconfig->lcm_thread_rate);
