...
bool SuperPoint::build() {
  ...
  auto builder = tensorrt_common::TensorRTUniquePtr<nvinfer1::IBuilder>(
      nvinfer1::createInferBuilder(tensorrt_log::gLogger.getTRTLogger()));
  if (!builder) {
    return false;
  }
  ...
  auto network =
      tensorrt_common::TensorRTUniquePtr<nvinfer1::INetworkDefinition>(
          builder->createNetworkV2(explicit_batch));
  if (!network) {
    return false;
  }
  auto config = tensorrt_common::TensorRTUniquePtr<nvinfer1::IBuilderConfig>(
      builder->createBuilderConfig());
  if (!config) {
    return false;
  }
  auto parser = tensorrt_common::TensorRTUniquePtr<nvonnxparser::IParser>(
      nvonnxparser::createParser(*network,
                                 tensorrt_log::gLogger.getTRTLogger()));
  if (!parser) {
    return false;
  }
  ...
  tensorrt_common::TensorRTUniquePtr<IHostMemory> plan{
      builder->buildSerializedNetwork(*network, *config)};
  if (!plan) {
    return false;
  }
  tensorrt_common::TensorRTUniquePtr<IRuntime> runtime{
  ...
  if (super_point_config_.enable_pca) {
    if (super_point_config_.pca_mean_path.empty() ||
        super_point_config_.pca_comp_path.empty()) {
      ...
      pca_comp_T =
          load_csv_mat_eigen(super_point_config_.pca_comp_path).transpose();
      pca_mean =
          load_csv_vec_eigen(super_point_config_.pca_mean_path).transpose();
    }
  } else {
    pca_comp_T_.resize(0, 0);
    pca_mean_.resize(0);
...
bool SuperPoint::constructNetwork(
    tensorrt_common::TensorRTUniquePtr<nvinfer1::IBuilder> &builder,
  ...
  auto parsed = parser->parseFromFile(
      super_point_config_.onnx_path.c_str(),
...
bool SuperPoint::infer(const cv::Mat &image,
  ...
  if (!context_) {
    context_ = tensorrt_common::TensorRTUniquePtr<nvinfer1::IExecutionContext>(
...
bool SuperPoint::infer(const cv::Mat &input,
...
bool SuperPoint::processInput(const tensorrt_buffer::BufferManager &buffers,
                              const cv::Mat &image) {
  ...
  memcpy(host_data_buffer, mono_image.data,
         mono_image.rows * mono_image.cols * sizeof(float));
  return true;
}
...
void SuperPoint::removeBorders(std::vector<Eigen::Vector2f> &keypoints,
  ...
  for (int i = 0; i < keypoints.size(); ++i) {
    bool flag_h = (keypoints[i][1] >= border) &&
                  (keypoints[i][1] < (height - border));  // v
    bool flag_w = (keypoints[i][0] >= border) &&
                  (keypoints[i][0] < (width - border));  // u
    if (flag_h && flag_w) {
...
void SuperPoint::topKeypoints(std::vector<Eigen::Vector2f> &keypoints,
...
void normalize_keypoints(const std::vector<Eigen::Vector2f> &keypoints,
                         std::vector<Eigen::Vector2f> &keypoints_norm, int h,
                         int w, int s) {
...
void SuperPoint::sampleDescriptors(std::vector<Eigen::Vector2f> &keypoints,
                                   float *des_map,
                                   std::vector<Eigen::VectorXf> &descriptors,
...
bool SuperPoint::processOutput(const tensorrt_buffer::BufferManager &buffers,
                               std::vector<Eigen::Vector2f> &keypoints,
                               std::vector<Eigen::VectorXf> &descriptors,
                               std::vector<float> &scores) {
  keypoints.clear();
  ...
  findHighScoreIndex(scores_vec, keypoints, semi_feature_map_h,
                     semi_feature_map_w,
                     super_point_config_.keypoint_threshold);
  removeBorders(keypoints, scores_vec, super_point_config_.remove_borders,
  ...
  sampleDescriptors(keypoints, output_desc, descriptors, desc_feature_dim,
...
bool SuperPoint::deserializeEngine() {
  ...
  spdlog::warn("[Superpoint]: deserialize engine failed; try to read from {}",
               super_point_config_.engine_path);
  return false;
}

// void SuperPoint::computeDescriptors(const std::vector<cv::Point2f>&
// keypoints,
//                         float *des_map,
...
// //des_map to mDesc
//   auto mDesc = torch::from_blob(des_map, {1, kSuperPointDescDim, height / 8,
//   width / 8}, torch::kFloat);
...
//   auto grid = torch::zeros({1, 1, fkpts.size(0), 2});  // [1, 1, n_keypoints,
//   2] grid[0][0].slice(1, 0, 1) = 2.0 * fkpts.slice(1, 1, 2) / width - 1;   //
//   x grid[0][0].slice(1, 1, 2) = 2.0 * fkpts.slice(1, 0, 1) / height - 1;  //
//   y
