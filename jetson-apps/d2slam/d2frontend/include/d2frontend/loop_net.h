...
class LoopNet {
  ...
  void onLoopConnectionRecevied(const lcm::ReceiveBuffer* rbuf,
                                const std::string& chan, const LoopEdge_t* msg);
  ...
  void processRecvImageDesc(const ImageDescriptor_t& image,
  ...
  bool msgBlocked(int64_t _id) {
    return blacklist.find(_id) != blacklist.end() ||
           sent_message.find(_id) != sent_message.end();
  }
 ...
 public:
  ...
  LoopNet(std::string _lcm_uri, bool _send_img, bool _send_whole_img_desc,
          double _recv_period = 0.5)
      : lcm(_lcm_uri),
        send_img(_send_img),
        send_whole_img_desc(_send_whole_img_desc),
        recv_period(_recv_period) {
  ...
  int lcmHandle() { return lcm.handle(); }
};
}  // namespace D2FrontEnd
