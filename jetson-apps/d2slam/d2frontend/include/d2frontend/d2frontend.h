...
class D2Frontend {
 ...
 protected:
  ...
  void stereoImagesCallback(const sensor_msgs::ImageConstPtr left,
                            const sensor_msgs::ImageConstPtr right);
  void depthImagesCallback(const sensor_msgs::ImageConstPtr left,
  ...
  void startThread() {
    ...
    stereo_frame_thread_ =
        std::thread(&D2Frontend::processStereoFrameThread, this);
    loop_detection_thread_running_ = true;
    loop_detection_thread_ =
        std::thread(&D2Frontend::loopDetectionThread, this);
 ...
 public:
  D2Frontend();
  virtual Swarm::Pose getMotionPredict(double stamp) const {
    return Swarm::Pose();
  };
  void stopFrontend() { stopThreads(); };
};
