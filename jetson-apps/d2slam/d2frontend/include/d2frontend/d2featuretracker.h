...
struct D2FTConfig {
  ...
  bool enable_search_local_aera_remote =
      false;  // Enable motion prediction searching for remote drones.
  ...
  double landmark_distance_assumption =
      100.0;  // For uninitialized landmark, assume it is 100 meter away
  int frame_step = 2;
...
struct TrackReport {
  ...
  double meanParallex() const { return sum_parallex / parallex_num; }
};
...
class D2FeatureTracker {
 protected:
  ...
  int frame_count = 0;
  bool inited = false;
  std::map<int, std::map<int, LKImageInfoGPU>>
      keyframe_lk_infos;  // frame.camera_index->image
  std::pair<bool, LandmarkPerFrame> createLKLandmark(
      const VisualImageDesc& frame, cv::Point2f pt,
      LandmarkIdType landmark_id = -1,
      LandmarkType type = LandmarkType::FlowLandmark);
  ...
  TrackReport track(const VisualImageDesc& left_frame,
                    VisualImageDesc& right_frame, bool enable_lk = true,
                    TrackLRType type = WHOLE_IMG_MATCH,
                    bool use_lk_for_left_right_track = true);
  TrackReport trackLK(const VisualImageDesc& frame,
                      VisualImageDesc& right_frame,
                      TrackLRType type = WHOLE_IMG_MATCH,
                      bool use_lk_for_left_right_track = true);
  TrackReport track(VisualImageDesc& frame,
                    const Swarm::Pose& motion_prediction = Swarm::Pose());
  TrackReport trackRemote(VisualImageDesc& frame,
                          const VisualImageDesc& prev_frame,
                          bool use_motion_predict = false,
                          const Swarm::Pose& motion_prediction = Swarm::Pose());
  bool getMatchedPrevKeyframe(const VisualImageDescArray& frame_a,
                              VisualImageDescArray& prev, int& dir_a,
                              int& dir_b);
  ...
  std::pair<bool, LandmarkPerFrame> getPreviousLandmarkFrame(
      const LandmarkPerFrame& lpf, FrameIdType keyframe_id = -1) const;
  const VisualImageDescArray& getLatestKeyframe() const;

  void draw(const VisualImageDesc& frame, bool is_keyframe,
            const TrackReport& report) const;
  void draw(const VisualImageDesc& lframe, VisualImageDesc& rframe,
            bool is_keyframe, const TrackReport& report) const;
  void draw(const VisualImageDescArray& frames, bool is_keyframe,
            const TrackReport& report) const;
  void drawRemote(const VisualImageDescArray& frames,
                  const TrackReport& report) const;
  void cvtRemoteLandmarkId(VisualImageDesc& frame) const;
  cv::Mat drawToImage(const VisualImageDesc& frame, bool is_keyframe,
                      const TrackReport& report, bool is_right = false,
                      bool is_remote = false) const;
  std::unordered_map<LandmarkIdType, LandmarkIdType>
      remote_to_local;  // Remote landmark id to local;
  std::unordered_map<LandmarkIdType, std::unordered_map<int, LandmarkIdType>>
      local_to_remote;  // local landmark id to remote drone and id;
  typedef std::lock_guard<std::recursive_mutex> Guard;
  SuperGlueOnnx* superglue = nullptr;
  bool matchLocalFeatures(const VisualImageDesc& img_desc_a,
                          const VisualImageDesc& img_desc_b,
                          std::vector<int>& ids_down_to_up,
                          const MatchLocalFeatureParams& param);
  std::map<LandmarkIdType, cv::Point2f> predictLandmarksWithExtrinsic(
  ...
  std::vector<cv::Point2f> predictLandmarks(const VisualImageDesc& img_desc_a,
                                            const Swarm::Pose& cam_pose_a,
                                            const Swarm::Pose& cam_pose_b,
                                            bool use_extrinsic = false) const;
