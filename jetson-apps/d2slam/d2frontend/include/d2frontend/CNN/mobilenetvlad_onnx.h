...
class MobileNetVLADONNX : public ONNXInferenceGeneric {
 ...
 public:
  ...
  MobileNetVLADONNX(std::string engine_path, int _width, int _height,
                    bool use_tensorrt = true, bool use_fp16 = true,
                    bool use_int8 = false,
                    std::string int8_calib_table_name = "")
      : ONNXInferenceGeneric(engine_path, "image:0", "descriptor:0", _width,
                             _height, use_tensorrt, use_fp16, use_int8,
    ...
    std::cout << "Trying to init MobileNetVLADONNX@" << engine_path
              << " tensorrt " << use_tensorrt << " fp16 " << use_fp16
              << " int8 " << use_int8 << " pca " << params->enable_pca_netvlad
              << std::endl;
  ...
  std::vector<float> inference(const cv::Mat& input) {
    ...
    if (pca_comp_T.rows() > 0) {
      ...
      return std::vector<float>(desc_pca.data(),
                                desc_pca.data() + desc_pca.size());
    }
