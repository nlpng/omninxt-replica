...
class ONNXInferenceGeneric : public CNNInferenceGeneric {
 ...
 public:
  ...
  virtual void doInference(const unsigned char* input,
                           const uint32_t batchSize) override {
  ...
  void init(const std::string& engine_path, bool onnx_with_tensorrt,
            bool enable_fp16, bool enable_int8,
            std::string int8_calib_table_name = "") {
    ...
    if (onnx_with_tensorrt) {
      ...
      memcpy(int8_calib_table_name_c, int8_calib_table_name.c_str(),
             int8_calib_table_name.size());
      ...
      tensorrt_options.trt_int8_calibration_table_name =
          int8_calib_table_name_c;
      tensorrt_options.trt_dump_subgraphs = 0;
      session_options.AppendExecutionProvider_TensorRT(tensorrt_options);
      printf(
          "ONNX will use TensorRT for inference INT8 %d FP16 %d cache path "
          "%s\n",
