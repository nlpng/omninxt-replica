...
class SuperPoint {
 public:
  ...
  bool build();
  // bool infer(const cv::Mat &image, Eigen::Matrix<double, 259, Eigen::Dynamic>
  // &features);
  bool infer(const cv::Mat &image, std::vector<Eigen::Vector2f> &keypoints,
             std::vector<Eigen::VectorXf> &descriptors,
             std::vector<float> &scores);

  bool infer(const cv::Mat &input, std::vector<cv::Point2f> &keypoints,
             std::vector<float> &local_descriptors,
             std::vector<float> &scores);  // a middle level for D2SLAM
 ...
 private:
  ...
  // Eigen::Matrix<float, kSuperPointDescDim, Eigen::Dynamic> &features);
  ...
  void removeBorders(std::vector<Eigen::Vector2f> &keypoints,
                     std::vector<float> &scores, int border, int height,
                     int width);

  std::vector<size_t> sortIndexes(std::vector<float> &data);

  void topKeypoints(std::vector<Eigen::Vector2f> &keypoints,
                    std::vector<float> &scores, int k);

  void findHighScoreIndex(std::vector<float> &scores,
                          std::vector<Eigen::Vector2f> &keypoints, int h, int w,
                          float threshold);

  void sampleDescriptors(std::vector<Eigen::Vector2f> &keypoints,
                         float *des_map,
                         std::vector<Eigen::VectorXf> &descriptors, int dim,
                         int h, int w, int s = 8);
};
