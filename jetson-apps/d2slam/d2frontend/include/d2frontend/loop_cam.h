...
class LoopCam {
 ...
 public:
  ...
  VisualImageDesc extractorImgDescDeepnet(ros::Time stamp, cv::Mat img,
                                          int index, int camera_id,
                                          bool superpoint_mode = false);
  std::vector<VisualImageDesc> generateStereoImageDescriptor(
      const StereoFrame &msg, int i, cv::Mat &_show);
  VisualImageDesc generateGrayDepthImageDescriptor(const StereoFrame &msg,
                                                   int i, cv::Mat &_show);
  VisualImageDesc generateImageDescriptor(const StereoFrame &msg, int i,
                                          cv::Mat &_show);
  ...
  CameraConfig getCameraConfiguration() const { return camera_configuration; }
};
}  // namespace D2FrontEnd
