...
class LoopDetector {
 ...
 protected:
  ...
  bool computeLoop(const VisualImageDescArray &frame_array_a,
                   const VisualImageDescArray &frame_array_b, int main_dir_a,
                   int main_dir_b, LoopEdge &ret);

  bool computeCorrespondFeatures(const VisualImageDesc &new_img_desc,
                                 const VisualImageDesc &old_img_desc,
  ...
  bool computeCorrespondFeaturesOnImageArray(
  ...
  int addImageArrayToDatabase(VisualImageDescArray &new_fisheye_desc,
                              bool add_to_faiss = true);
  int addImageDescToDatabase(VisualImageDesc &new_img_desc);
  bool queryImageArrayFromDatabase(const VisualImageDescArray &new_img_desc,
                                   VisualImageDescArray &ret,
                                   int &camera_index_new,
                                   int &camera_index_old);
  int queryFrameIndexFromDatabase(const VisualImageDesc &new_img_desc,
                                  double &similarity);
  int queryIndexFromDatabase(const VisualImageDesc &new_img_desc,
                             faiss::IndexFlatIP &index, bool remote_db,
                             double thres, int max_index, double &similarity);

  bool checkLoopOdometryConsistency(LoopEdge &loop_conn) const;
  void drawMatched(const VisualImageDescArray &fisheye_desc_a,
                   const VisualImageDescArray &fisheye_desc_b, int main_dir_a,
                   int main_dir_b, bool success, std::vector<int> inliers,
                   Swarm::Pose DP_b_to_a,
                   std::vector<std::pair<int, int>> index2dirindex_a,
                   std::vector<std::pair<int, int>> index2dirindex_b);
