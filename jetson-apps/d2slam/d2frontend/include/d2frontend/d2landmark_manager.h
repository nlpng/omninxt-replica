...
class LandmarkManager {
 ...
 public:
  ...
  const LandmarkPerId& at(LandmarkIdType i) const { return landmark_db.at(i); }
  std::vector<LandmarkPerId> popFrame(
      FrameIdType frame_id,
      bool pop_base = false);  // If pop base, we will remove the related
                               // landmarks' base frame.
